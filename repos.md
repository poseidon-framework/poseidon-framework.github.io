...
6. Add/update the file checksums in the `POSEIDON.yml` file with [`trident checksumupdate`](trident?id=checksumupdate-command)
...
2. Modify the files you want to change. Remember to also update the md5 checksums in the POSEIDON.yml file after you are done. This can be triggered with [`trident checksumupdate`](trident?id=checksumupdate-command).
